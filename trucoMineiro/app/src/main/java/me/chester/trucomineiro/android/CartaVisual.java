...
public class CartaVisual extends Carta {
	...
	/**
	 * Utiliza uma carta criada dinamicamente (acaba com os resources!!)
	 */
	/**
	...
	public void movePara(int left, int top, int tempoMS) {
		...
		destTime = ultimoTime + tempoMS + 150;
	...
	private Bitmap getBitmap() {
		if (this.bitmap == null || this.bitmap.isRecycled()) {
			...
			/**
			 * Aqui vou mudar o método para criar a carta ao invés de 
			 * buscar nos recursos
			 */
			
			Bitmap.Config conf = Bitmap.Config.ARGB_8888; // see other conf types
			this.bitmap = Bitmap.createBitmap(largura, altura, conf);
			...
			Rect rect = new Rect(0, 0,  largura - 1, altura - 1);
			RectF rectf = new RectF(rect);
			...
			if ((!isFechada()) && (getLetra() != LETRA_NENHUMA)
				...
				alinharTexto("" + letra, 0, 0, largura / 2, altura / 3, altura / 4, fCor, canvas);
				alinharTexto(fNaipes[naipe % 4], 0, altura / 8, largura, altura - altura / 8, altura / 2, fCor, canvas);
			} else {
	...
	/**private static int getCartaResourceByValor(String valor) {
		...
		try {
			for (Class c : Class.forName("me.chester.trucomineiro.R").getClasses()) {
