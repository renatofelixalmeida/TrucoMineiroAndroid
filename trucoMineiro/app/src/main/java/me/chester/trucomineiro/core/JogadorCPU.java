...
public class JogadorCPU extends Jogador implements Runnable {
	...
	public JogadorCPU(String nomeEstrategia) {
		this(criaEstrategiaPeloNome(nomeEstrategia, 1));
	}

	/**
	 * Cria um novo jogador CPU, com uma estratégia aleatória
	 */
	public JogadorCPU(int forca) {
		this(criaEstrategiaPeloNome("x", forca));
	}
	...
	public void vez(Jogador j, boolean podeFechada) {
		if (this.equals(j)) {
			this.podeFechada = podeFechada && this.jogo.pontosEquipe[0]!=10 && this.jogo.pontosEquipe[1]!=10;
			this.minhaVez = true;
	...
	public void run() {
		while (jogo == null || !jogo.jogoFinalizado) {
			...
			if (recebiPedidoDeMaoDe10) {
				recebiPedidoDeMaoDe10 = false;
				atualizaSituacaoJogo();
				sleep(1000 + random.nextInt(1000));
				boolean respostaMao11 = false;
				try {
					respostaMao11 = estrategia.aceitaMao11(
							cartasDoParceiroDaMaoDe10, situacaoJogo);
				...
				jogo.decideMao10(this, respostaMao11);
			}
	...
	private boolean recebiPedidoDeMaoDe10 = false;

	private Carta[] cartasDoParceiroDaMaoDe10;
	...
	public void informaMao10(Carta[] cartasParceiro) {
		cartasDoParceiroDaMaoDe10 = cartasParceiro;
		recebiPedidoDeMaoDe10 = true;
	}
