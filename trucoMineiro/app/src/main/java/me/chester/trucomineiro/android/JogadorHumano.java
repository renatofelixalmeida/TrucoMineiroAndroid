...
public class JogadorHumano extends Jogador {
	...
	public void cartaJogada(Jogador j, Carta c) {
		mesa.mostrarPerguntaMao10 = false;
		...
		activity.jogo.jogadorQueJogou = j;
	}

	@Override
	public void decidiuMao10(Jogador j, boolean aceita) {
	...
	@Override
	public void informaMao10(Carta[] cartasParceiro) {
		mesa.mostraCartasMao10(cartasParceiro);
		mesa.mostrarPerguntaMao10 = true;
	}
	...
	public void inicioMao() {
		...
		// aqui tá foda, eu tenho que fazer o jogador que ganhou o jogo falar isso
		// tenho que pegar um jogador da equipe vencedora aletaorio para isso
		int jogadorMsg = (new Random()).nextInt(4) + 1;
		String msg = "";  

		// verificar se a equipe 1 fez jogo
		if (activity.jogo.fezJogo[0]) {
			if (activity.jogo.getJogador(jogadorMsg).getEquipe() != 1) {
				jogadorMsg++;
				if (jogadorMsg>4) jogadorMsg = 1;
			}
		}

		// agora verificar se a equipe 2 fez jogo
		if (activity.jogo.fezJogo[1]) {
			if (activity.jogo.getJogador(jogadorMsg).getEquipe() != 2) {
				jogadorMsg++;
				if (jogadorMsg>4) jogadorMsg = 1;
			}
		}
		
		// verificar se algum dos dois fez jogo e exibir a mensagem
		if (activity.jogo.fezJogo[0]||activity.jogo.fezJogo[1]) {
			// agora verificar se está virando ou fazendo jogo
			if (activity.jogo.jogos[0]==1&&activity.jogo.jogos[1]==1) {
				msg = "empata_jogo";
			} else {
				msg = "ganha_jogo";
			}
			mesa.diz(msg, posicaoNaTela(activity.jogo.getJogador(jogadorMsg)), 1500);
		}
		
		mesa.distribuiMao();
		
		mesa.sinal = activity.jogo.jogadores[2].getSinal();
		
		if (activity.sinalForca>=mesa.sinal) {
			long temsinal = System.currentTimeMillis() + 1500;
			if (activity.jogo.pontosEquipe[0]!=10){
				mesa.exibirSinal = temsinal;
			} else if (activity.jogo.pontosEquipe[1]==10){
				mesa.exibirSinal = temsinal;			
			} else mesa.exibirSinal = System.currentTimeMillis();	
			mesa.notificaAnimacao(mesa.exibirSinal + 500);
		}
		
		if (activity.jogo.pontosEquipe[0]==0&&activity.jogo.pontosEquipe[1]==0&&
				activity.jogo.jogos[0]+activity.jogo.jogos[0]>0)
	...
	public void jogoFechado(int numEquipeVencedora) {
		...
		// pegar um jogador qualquer para falar que ganhou
		int jogadorMsg = (new Random()).nextInt(4) + 1;

		// verificar se o jogador é da equipe que ganhou
		if (activity.jogo.getJogador(jogadorMsg).getEquipe() != numEquipeVencedora) {
			jogadorMsg++;
			if (jogadorMsg>4) jogadorMsg = 1;
		}
		
		// exibir a mensagem
		mesa.diz("vitoria", posicaoNaTela(activity.jogo.getJogador(jogadorMsg)), 1500);
	...
	public void vez(Jogador j, boolean podeFechada) {
		...
		boolean mostraBtnAumento = (j instanceof JogadorHumano)
				&& (valorProximaAposta > 0) && (activity.placar[0] != 10)
				&& (activity.placar[1] != 10);
		boolean mostraBtnAbertaFechada = (j instanceof JogadorHumano)
				&& podeFechada  && (activity.placar[0] != 10)
				&& (activity.placar[1] != 10);
		activity.handler.sendMessage(Message.obtain(activity.handler,
	...
	private void incrementaEstatistica(String chave) {
		...
		int jogador = Integer.parseInt(preferences.getString("meuJogador", "0"));
		int partidasJogador = preferences.getInt(chave + jogador, 0);
		int partidas = preferences.getInt(chave, 0);
		Editor editor = preferences.edit();
		editor.putInt(chave, ++partidas);
		editor.putInt(chave + jogador, ++partidasJogador);
		// pegar o revezamento do jogador
		boolean vitoria = chave.indexOf("itor") > 0;
		
		jogador++;

		if (jogador==4) jogador = 0;
		
		switch(Integer.parseInt(preferences.getString("revezar", "0"))){
			case 1:
				editor.putString("meuJogador", "" + (jogador));
				break;
			case 2:
				if (vitoria)editor.putString("meuJogador", "" + (jogador));					
				break;
			case 3:
				if (!vitoria) editor.putString("meuJogador", "" + (jogador));
				break;
