...
public class EstrategiaRenato implements Estrategia {
	private static Random rand = new Random();

	int[] C = new int[3];
	int[] INICIOU = new int[3];
	...
	private String[] nicks_estrategia = { "HAL", "nero(HAL_PRO)", "eliza(HAL_PRO)", "alex(HAL_PRO)", "constantine(HAL_PRO)" };
	 ...
	 */
	private boolean mandaBala(int fatorDeDuvida, int reducao){
		// this is where we adjust agressiveness as per the different "personas"
		if (gameLevel == 0)
			fatorDeDuvida = fatorDeDuvida - (int) ( 15 / reducao);
		if (gameLevel == 1)
			fatorDeDuvida = fatorDeDuvida - (int) ( 5 / reducao);
		if (gameLevel == 2)
			fatorDeDuvida = fatorDeDuvida + (int) ( 5 / reducao);		
		if (gameLevel == 3)
			fatorDeDuvida = fatorDeDuvida + (int) (17 / reducao);
		if (gameLevel == 4)
			fatorDeDuvida = fatorDeDuvida + (int) (30 / reducao);		
	...
	private boolean mandaBala(int fatorDeDuvida) {
		...
		if (gameLevel == 1)
			fatorDeDuvida = fatorDeDuvida - 5;
		if (gameLevel == 2)
			fatorDeDuvida = fatorDeDuvida + 5;		
		if (gameLevel == 3)
			fatorDeDuvida = fatorDeDuvida + 17;
		if (gameLevel == 4)
			fatorDeDuvida = fatorDeDuvida + 30;		
		if (fatorDeDuvida > 100)
			fatorDeDuvida = 100;
		if (fatorDeDuvida < 0)
			fatorDeDuvida = 0;
	...
	/*
	 *  Verifica a possibilidade de ter uma carta maior
	 */

	private int chanceMaior(SituacaoJogo s, int valorCarta, boolean podeEmpatar){
		return chanceMaior(s, valorCarta, podeEmpatar, 100);
	}
	

	private int chanceMaior(SituacaoJogo s, Carta carta, boolean podeEmpatar){	
		int valorCarta = carta.getValorTruco(s.manilha);	
		return chanceMaior(s, valorCarta, podeEmpatar, 100);
	}
	

	private int chanceMaior(SituacaoJogo s, Carta carta, boolean podeEmpatar, int variacao){	
		int valorCarta = carta.getValorTruco(s.manilha);	
		return chanceMaior(s, valorCarta, podeEmpatar, variacao);
	}
	
	private int chanceMaior(SituacaoJogo s, int valorCarta, boolean podeEmpatar, int variacao){
		int cartasJogo[] = {4, 4, 4, 4, 4, 4, 4, 4, 
							4, 4, 4, 4, 4, 4, 4};
		if (podeEmpatar) valorCarta++;
		int total = 0;
		int menor = 0;
		int i = 0;
		int j = 0;
		
		// marcas as cartas atuais
		cartasJogo[1] = 3;
		cartasJogo[4] = 2;
		cartasJogo[8] = 3;
		cartasJogo[11] = 1;
		...
		// marca as cartas que sairam
		for (i=0;i<s.numRodadaAtual;i++){
			for (j=0;j<4;j++){
				if (s.cartasJogadas[i][j]!=null) {
					cartasJogo[s.cartasJogadas[i][j].getValorTruco(s.manilha)]--;
				}
			}
		}
		...
		for (i=1;i<15;i++){
			total = total + cartasJogo[i];
			if (valorCarta>i) menor = menor + cartasJogo[i];
		}
		
		menor = menor * variacao;
		
		return (int) menor / total;
	}
	/**
	 * Tentar supor a maior e menor carta do seu parceiro e adversários
	 * Tenho que criar algumas variaveis para definir as seguintes situações
	 * 1 - Jogador que abriu a rodada
	 * 2 - Jogador que fez a rodada
	 */
	
	// gravar qual jogador (0-3) abriu a rodada (0-2);
	private int [] abriuRodada = new int[3];
	
	// supõe a maior e menor carta de cada jogador
	private int [] maiorCarta = new int[4];
	private int [] menorCarta = new int[4];
	
	// grava qual jogador fez a rodada só para facilitar
	private int [] fezRodada = new int[4];
	
	private void suporCartas(SituacaoJogo s) {
		// Gravar quem abriu a rodada, até da terceira para evitar fazer if
		abriuRodada[s.numRodadaAtual-1] = s.posJogadorQueAbriuRodada - 1;
		
		// quando for a primeira rodada vou zerar tudo
		if (s.numRodadaAtual==1) {
			maiorCarta[0] = 20;
			maiorCarta[1] = 20;
			maiorCarta[2] = 20;
			maiorCarta[3] = 20;
		...
		//if (s.posJogador==2) Log.d("renato", "Abriu rodada: " + abriuRodada[s.numRodadaAtual-1]);
		
		// na segunda e terceira rodadas vou fazer um loop sobre as cartas da primeira e segunda para ver quem fez
		if (s.numRodadaAtual>1) {
			int lMaior = 0;
			for (int i=0;i<4;i++) {
				if (s.cartasJogadas[s.numRodadaAtual-2][i].getValorTruco(s.manilha)>=lMaior) {
					fezRodada[s.numRodadaAtual-2] = i;
					lMaior = s.cartasJogadas[s.numRodadaAtual-2][i].getValorTruco(s.manilha);
				}
			}
			if (s.posJogador==5) {
				Log.d("renato", "fez :" + (s.numRodadaAtual) + " " + fezRodada[s.numRodadaAtual-2]);
			}
			// agora vou avaliar algumas situações
			// se o jogador é pé e fez uma carta baixa com carta muito alta ele deve ter só uma
			// neste caso a sua maior carta é a que veio
			boolean jaFez = false;

			for (int i=abriuRodada[s.numRodadaAtual - 2];i<abriuRodada[s.numRodadaAtual - 2] + 4;i++) {		
				// se algum jogador já fez a rodada, obviamente os outros jogadores vão descartar as menores
				// e tambem não tem nenhuma maior, senão eles faziam
				if (jaFez) {
					menorCarta[i%4] = s.cartasJogadas[s.numRodadaAtual-2][i%4].getValorTruco(s.manilha)>0?s.cartasJogadas[s.numRodadaAtual-2][i%4].getValorTruco(s.manilha): menorCarta[i%4];
					maiorCarta[i%4] = lMaior;
				}
				
				// se foi ele quem fez vamos marcar para avisar os outros
				if (fezRodada[s.numRodadaAtual-2]==i%4) jaFez = true;
			}
			// agora eu vou ver se o jogador que fez é o pé, se for e ele fez uma porcaria com uma carta alta, 
			// a porcaria fica sendo a maior dele
			if (fezRodada[s.numRodadaAtual-2]==(abriuRodada[s.numRodadaAtual-2] + 3)%4){
				int carta1 = s.cartasJogadas[s.numRodadaAtual-2][(abriuRodada[s.numRodadaAtual-2] + 3)%4].getValorTruco(s.manilha);
				int carta2 = s.cartasJogadas[s.numRodadaAtual-2][abriuRodada[s.numRodadaAtual-2]].getValorTruco(s.manilha);
				int carta3 = s.cartasJogadas[s.numRodadaAtual-2][(abriuRodada[s.numRodadaAtual-2]+2)%4].getValorTruco(s.manilha);
				if (carta2>carta3) carta3=carta2;
				if (carta1-carta3>5) maiorCarta[fezRodada[s.numRodadaAtual-2]] = carta3;
			} 
		}	
	}
	...
	private int joga_primeira_rodada_mao(SituacaoJogo s) {
		...
		if (s.pontosEquipe[adversario1(s) % 2] < 6
			...
			if (gameLevel != 0 && mandaBala(0, 3)) return -1;
		}
	...
	private boolean aceita_primeira_rodada_mao(SituacaoJogo s) {
		// aqui nem tem jeito, coloquei esse só para ficar padronizado
		return false;
	}
	...
	private int joga_primeira_rodada_posmao(SituacaoJogo s) {
		...
		if (qualidadeMaiorMesa(s) == LIXO && mandaBala(50, 2)) {
			// Aqui vamos jogar a menor manilha (menos o zap)
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == PICAFUMO)
				return C[0];
			if (qualidadeCarta(s.cartasJogador[C[1]], s) == PICAFUMO)
	...
	private boolean aceita_primeira_rodada_posmao(SituacaoJogo s) {
		// o cara já começou trucando, não tem muito o que fazer, vou dar uma avaliada nas minhas cartas e ver
		// se eu tiver um zap, três ou maior vou aceitar
		if (s.cartasJogador[C[0]].getValorTruco(s.manilha)==ZAP &&
			s.cartasJogador[C[1]].getValorTruco(s.manilha)>=TRES) return true;
		// se tiver 2 manilhas vou aceitar
		if (s.cartasJogador[C[1]].getValorTruco(s.manilha)>TRES) return true;
		// set tiver 2 tres ou manilha vou aceitar com probabilidade alta
		if (s.cartasJogador[C[1]].getValorTruco(s.manilha)>DOIS &&
			this.mandaBala(80)) return true;
		// se tiver 2 dois ou mais vou aceitar com probabilidade media-alta
		if (s.cartasJogador[C[1]].getValorTruco(s.manilha)>=DOIS &&
			this.mandaBala(55)) return true;
		// se tiver 1 manilha pelo menos vou aceitar com probabilidade media-alta pois nosso parceiro vai ajudar
		if (s.cartasJogador[C[0]].getValorTruco(s.manilha)>TRES && 
			this.mandaBala(70)) return true;
		// set tiver pelo menos figura vou aceitar com probabilidade baixa
		if (s.cartasJogador[C[1]].getValorTruco(s.manilha)>5 && 
				this.mandaBala(5)) return true;		
		// se tiver só porcaria vou aceitar com probabilidade baixa pois o cara pode ser um ladrão safado
		if (this.mandaBala(20,2)) return true;
		// vou correr, o negócio tá feio
		return false;
	}
	...
	private boolean aceita_primeira_rodada_parape(SituacaoJogo s) {
		return false;
	}
	...
	private boolean aceita_primeira_rodada_pe(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_segunda_rodada_mao(SituacaoJogo s) {
		...
		if (rodadaAmarrou(s, 0)) {
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ZAP	&& s.valorProximaAposta>0)	return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ESCOPETA && podeEValeAPenaAumentar(s, 90, 75, 60, 1)) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ESPADILHA && podeEValeAPenaAumentar(s, 80, 60, 50, 1)) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == PICAFUMO && podeEValeAPenaAumentar(s, 70, 50, 30, 1)) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES && podeEValeAPenaAumentar(s, 40, 30, 10, 1)) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == DOIS && podeEValeAPenaAumentar(s, 30, 20, 5, 1)) return -1;			
			return C[0];
		}
	...
	private boolean aceita_segunda_rodada_mao(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_segunda_rodada_posmao(SituacaoJogo s) {
		...
		if (rodadaAmarrou(s, 0)) {
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ZAP	&& s.valorProximaAposta>0)	return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ESCOPETA && podeEValeAPenaAumentar(s, 90, 75, 60, 1) && this.qualidadeMaiorMesa(s)<ESCOPETA) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == ESPADILHA && podeEValeAPenaAumentar(s, 80, 60, 50, 1) && this.qualidadeMaiorMesa(s)<ESPADILHA) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == PICAFUMO && podeEValeAPenaAumentar(s, 70, 50, 30, 1) && this.qualidadeMaiorMesa(s)<PICAFUMO) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES && podeEValeAPenaAumentar(s, 40, 30, 10, 1) && this.qualidadeMaiorMesa(s)<TRES) return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) == DOIS && podeEValeAPenaAumentar(s, 30, 20, 5, 1) && this.qualidadeMaiorMesa(s)<DOIS) return -1;			
			if (qualidadeCarta(s.cartasJogador[C[0]], s) >= qualidadeMaiorMesa(s)) return C[0];
		}
		return C[1];
	}

	private boolean aceita_segunda_rodada_posmao(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_segunda_rodada_parape(SituacaoJogo s) {
		...
		if (qualidadeCarta(s.cartasJogador[C[0]], s) >= TRES
			...
			if (podeEValeAPenaAumentar(s, 60, 30, 1, 0)
		...
		if (qualidadeCarta(s.cartasJogador[C[0]], s) == AS
				&& qualidadeCarta(s.cartasJogador[C[0]], s) >= qualidadeMaiorMesa(s)) {
			if (podeEValeAPenaAumentar(s, 40, 10, 1, 0)
					&& !partidaGanhaParaAdversario(s))
				return -1;
			return C[0];
		}
		...
		if (qualidadeCarta(s.cartasJogador[C[0]], s) == LIXO) {
			// facão
			if (podeEValeAPenaAumentar(s, 10, 5, 1, 0)
					&& !partidaGanhaParaAdversario(s)) {
				return -1;
			}
			if (qualidadeCarta(s.cartasJogador[C[0]], s) >= qualidadeMaiorMesa(s))
	...
	private boolean aceita_segunda_rodada_parape(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_segunda_rodada_pe(SituacaoJogo s) {
		...
		// Se a 1a amarrou, jogar a maior
		if (rodadaAmarrou(s, 0)) {
			if (qualidadeCarta(s.cartasJogador[C[0]], s) > qualidadeMaiorMesa(s) && parceiro(s) != maiorCartaMesa(s) && podeEValeAPenaAumentar(s, 100, 100, 100, 100))	return -1;
			if (qualidadeCarta(s.cartasJogador[C[0]], s) >= qualidadeMaiorMesa(s) && parceiro(s) != maiorCartaMesa(s))	return C[0];
			return C[1];
		}

		if (maiorCartaEDoParceiro(s)){
			if 	(podeEValeAPenaAumentar(s, 20, 10, 5, 1) && C[0]==ZAP) return -1; // ver se da pra trucar
			if 	(podeEValeAPenaAumentar(s, 15, 7, 4, -10) && C[0]==ESCOPETA) return -1; // ver se da pra trucar
			if 	(podeEValeAPenaAumentar(s, 10, 5, 3, -10) && C[0]==ESPADILHA) return -1; // ver se da pra trucar
			if 	(podeEValeAPenaAumentar(s, 8, 4, 2, -30) && C[0]==PICAFUMO) return -1; // ver se da pra trucar
	...
	private boolean aceita_segunda_rodada_pe(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_terceira_rodada_mao(SituacaoJogo s) {
		...
		if (s.cartasJogador[C[0]].getValorTruco(s.manilha)>maiorCarta[s.posJogador%4]&&
				s.cartasJogador[C[0]].getValorTruco(s.manilha)>maiorCarta[(s.posJogador+2)%4]) 
			forcar = 10;
		...
		if (qualidadeCarta(s.cartasJogador[C[0]], s) > TRES
				&& podeEValeAPenaAumentar(s, 45+forcar, 5+forcar, 1+forcar, 0)) {
			return -1;
		}
	...
	private boolean aceita_terceira_rodada_mao(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_terceira_rodada_posmao(SituacaoJogo s) {
		...
		if (s.cartasJogador[C[0]].getValorTruco(s.manilha)>maiorCarta[(s.posJogador+2)%4]) 
			forcar = 20;
		...
		if (qualidadeCarta(s.cartasJogador[C[0]], s) >= TRES
				&& podeEValeAPenaAumentar(s, forcar+45, forcar+10, forcar+1, 0)
				&& !partidaGanhaParaAdversario(s)) {
			return -1;
		}
	...
	private boolean aceita_terceira_rodada_posmao(SituacaoJogo s) {
		return false;
	}
	...
	private int joga_terceira_rodada_parape(SituacaoJogo s) {
		...
		if (maiorCartaEDoParceiro(s) && qualidadeMaiorMesa(s) >= TRES) {
			if (podeEValeAPenaAumentar(s,forcar + 10, forcar+2, forcar+1, 0))
				return -1;
		} else {
			if (qualidadeCarta(s.cartasJogador[C[0]], s) <= qualidadeMaiorMesa(s)) {
				if (podeEValeAPenaAumentar(s, forcar+20, forcar+5, forcar+1, 0)
						&& qualidadeMaiorMesa(s) != ZAP
						&& !partidaGanhaParaAdversario(s))
					return -1;
			} else {
				if (qualidadeCarta(s.cartasJogador[C[0]], s) > TRES
						&& podeEValeAPenaAumentar(s, 50+forcar, 5+forcar, 1+forcar, 0+forcar))
					return -1;

				if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES
						&& podeEValeAPenaAumentar(s, 25+forcar, 8+forcar, 1+forcar, 0))
					return -1;
			}
		}
	...
	private boolean aceita_terceira_rodada_parape(SituacaoJogo s) {
		return false;
	}
	...
	private boolean aceita_terceira_rodada_pe(SituacaoJogo s) {
		return false;
	}
	...
	private boolean aceita_primeira_rodada(SituacaoJogo s) {
		switch (minhaPosicao(s)) {
		// Mão
		case 0:
			return this.aceita_primeira_rodada_mao(s);
			// Pós mão
		case 1:
			return this.aceita_primeira_rodada_posmao(s);
			// Para pé
		case 2:
			return this.aceita_primeira_rodada_parape(s);
			// Pé
		case 3:
			return this.aceita_primeira_rodada_pe(s);
		}		
		return false;
	}
	
	private int joga_segunda_rodada(SituacaoJogo s) {
		// se a primeira empatou e eu tiver zap vou trucar
		switch (minhaPosicao(s)) {
		// Mão
		case 0:
	...
	private boolean aceita_segunda_rodada(SituacaoJogo s) {
		switch (minhaPosicao(s)) {
		// Mão
		case 0:
			return this.aceita_segunda_rodada_mao(s);
			// Pós mão
		case 1:
			return this.aceita_segunda_rodada_posmao(s);
			// Para pé
		case 2:
			return this.aceita_segunda_rodada_parape(s);
			// Pé
		case 3:
			return this.aceita_segunda_rodada_pe(s);
		}		
		return false;
	}

	private int joga_terceira_rodada(SituacaoJogo s) {
		// aqui é o seguinte, se tiver carta boa vou trucar mesmo
		if (qualidadeCarta(s.cartasJogador[C[0]],s)==ZAP&&s.valorProximaAposta>0) return -1;
		
		if (this.gameLevel>1){
			if (qualidadeCarta(s.cartasJogador[C[0]],s)==ESCOPETA&&s.valorProximaAposta>0&&
					this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)) return -1; 
			if (qualidadeCarta(s.cartasJogador[C[0]],s)==ESPADILHA&&s.valorProximaAposta>0&&
					this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)&&
					this.mandaBala(70)) return -1;	
			if (qualidadeCarta(s.cartasJogador[C[0]],s)==PICAFUMO&&s.valorProximaAposta>0&&
					this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)&&
					this.mandaBala(60)) return -1;	
			if (qualidadeCarta(s.cartasJogador[C[0]],s)==TRES&&s.valorProximaAposta>0&&
					this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)&&
					this.mandaBala(30)) return -1;	
			if (qualidadeCarta(s.cartasJogador[C[0]],s)==DOIS&&s.valorProximaAposta>0&&
					this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)&&
					this.mandaBala(10)) return -1;				
		}
		...
		case 3:
			...
			this.qualidadeMaiorMesa(s)<qualidadeCarta(s.cartasJogador[C[0]],s)) return -1;
			if (s.valorProximaAposta>0&&
			this.qualidadeMaiorMesa(s)<=AS&&mandaBala(6*gameLevel)) return -1; 			
			return this.joga_terceira_rodada_pe(s);			
		}
		return C[0]; // para satisfazer o corretor do método (Eclipse)...
	}

	private boolean aceita_terceira_rodada(SituacaoJogo s) {
		switch (minhaPosicao(s)) {
		// Mão
		case 0:
			return this.aceita_terceira_rodada_mao(s);
			// Pós mão
		case 1:
			return this.aceita_terceira_rodada_posmao(s);
			// Para pé
		case 2:
			return this.aceita_terceira_rodada_parape(s);
			// Pé
		case 3: 			
			return this.aceita_terceira_rodada_pe(s);			
		}
	...
	public int joga(SituacaoJogo s) {
		...
		// verificar a seguinte situação
		// o adversário já tem x pontos que somados com os pontos da mão já servem para ele ganhar
		// se eu puder aumentar vou trucar sem dó, só vou aceitar truco trucando!
	
		if ((s.valorProximaAposta>3&&s.pontosEquipe[adversario1(s) % 2]== 8)||
			(s.valorProximaAposta>7&&s.pontosEquipe[adversario1(s) % 2]>= 6)) {
			return -1;
		}
	...
	public boolean aceitaTruco(SituacaoJogo s) {
		...
		// se já dá pro adversário ganhar eu não vou correr, ele pode aumentar só para eu fugir
		if ((s.valorMao>3&&s.pontosEquipe[adversario1(s) % 2]== 8)||
			(s.valorMao>7&&s.pontosEquipe[adversario1(s) % 2]>= 6)) {
			return true;
		}
		
		if (s.valorMao+s.pontosEquipe[adversario1(s) % 2]>=12) return true;
		...
		case 1:
			...
			// aceito
			if (qualidadeCarta(s.cartasJogador[C[0]], s) >= DOIS
					&& maiorCartaENossa(s) && qualidadeMaiorMesa(s) >= DOIS&&
					mandaBala(50))
				return true;
			...
			return aceita_primeira_rodada(s);
			// segunda rodada
		case 2:
			...
			if (valeAPenaAceitar(s, 0))
				return true; // last check if there are any relevant cases we
								// should call before refuse it
			return aceita_segunda_rodada(s);
		...
		case 3:
			...
			if (primeiraENossa(s)) {
				...
				if (maiorCartaENossaPodendoEstarAmarrado(s)
						&& qualidadeMaiorMesa(s) > TRES
						&& valeAPenaAceitar(s, 70 + chanceMaior(s, qualidadeMaiorMesa(s), true, 20)))
					return true;
				// se na mesa já estiver um 3 nosso, aceito com média-alta prob.
				if (maiorCartaENossaPodendoEstarAmarrado(s)
						&& qualidadeMaiorMesa(s) == TRES
						&& valeAPenaAceitar(s, 50 + chanceMaior(s, qualidadeMaiorMesa(s), true, 20)))
					return true;
				// se na mesa já estiver um 2 nosso, aceito com média-baixa
				// prob.
				if (maiorCartaENossaPodendoEstarAmarrado(s)
						&& qualidadeMaiorMesa(s) == DOIS
						&& valeAPenaAceitar(s, 30 + chanceMaior(s, qualidadeMaiorMesa(s), true, 20)))
					return true;
				...
				if (minhaVez(s) > vezTrucador(s)
					...
					if (qualidadeCarta(s.cartasJogador[C[0]], s) > TRES
							&& valeAPenaAceitar(s, 70 + chanceMaior(s, qualidadeCarta(s.cartasJogador[C[0]], s), true, 25) ))
						return true;
					// se eu tenho um três, aceito com média-alta prob.
					if (qualidadeCarta(s.cartasJogador[C[0]], s) == TRES
							&& valeAPenaAceitar(s, 50 + chanceMaior(s, qualidadeCarta(s.cartasJogador[C[0]], s), true, 20) ))
						return true;
					// se eu tenho um dois, aceito com média-baixa prob.
					if (qualidadeCarta(s.cartasJogador[C[0]], s) == DOIS
							&& valeAPenaAceitar(s, 20 + chanceMaior(s, qualidadeCarta(s.cartasJogador[C[0]], s), true, 20) ))
						return true;
				}
			} else {
				// se a primeira NÃO for nossa:
				...
				// prob.
				if (maiorCartaENossa(s) && qualidadeMaiorMesa(s) > TRES
						&& valeAPenaAceitar(s, 70 + chanceMaior(s, qualidadeMaiorMesa(s), true, 25)))
					return true;
				...
				if (maiorCartaENossa(s) && qualidadeMaiorMesa(s) <= AS
						&& valeAPenaAceitar(s, 5 + chanceMaior(s, qualidadeMaiorMesa(s), true, 15)))
					return true;
				// checar se ainda tenho cartas na mão...
				if (minhaVez(s) > vezTrucador(s)
						&& !partidaGanhaParaAdversario(s)) {
			...
			if (valeAPenaAceitar(s, 0))
				return true; // last check if there are any relevant cases we
								// should call before refuse it
			return aceita_terceira_rodada(s);
		}
		return false;
	}
	...
	public boolean aceitaMao11(Carta[] cartasParceiro, SituacaoJogo s) {
		...
		if (s.maoDezAberta) {        // vamos analisar!
			...
			if ((qManilhas >= 1 || q3 >= 1 || q2 >= 1) && mandaBala(40)) {
				return true;
			}
		} else {

			if (qManilhas >= 2 || q3 >= 2 || (q3 >= 1 && q2 >= 2 && qManilhas >= 1)
					|| (qManilhas >= 1 && q3 >= 1) || (qManilhas >= 1 && q2 >= 1))
				return true;
			if (s.pontosEquipe[adversario1(s) % 2] > 6
					&& (qManilhas >= 1 && q2 >= 1) || q3 >= 2) {
			...
			if (s.pontosEquipe[adversario1(s) % 2] > 2
					&& (qManilhas >= 1 || q3 >= 1 || q2 >= 2)) {
				return true;
			}
			if ((qManilhas >= 1 || q3 >= 1 || q2 >= 1) && mandaBala(40)) {
